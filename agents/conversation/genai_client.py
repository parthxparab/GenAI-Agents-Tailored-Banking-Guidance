...
DEFAULT_MODEL = os.getenv("CONVERSATION_LLM_MODEL", "llama3")
...
def run_llm(prompt: str, model: Optional[str] = None) -> Dict[str, Any]:
    """Invoke a local Ollama model and attempt to parse the JSON response."""
    ...
    try:
        ...
        try:
            ...
            return {"error": f"Ollama CLI not found for model '{selected_model}'", "details": str(exc)}
        except Exception as exc:
            return {"error": f"Failed to invoke Ollama CLI for model '{selected_model}'", "details": str(exc)}
